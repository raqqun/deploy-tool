...
class Controller {
    ...
    public function deployToProduction($dryRun = false) {
        ...
        exec("rsync -avz".$dryRunOption." --itemize-changes --exclude-from '".$this->absPathToConfig."/rsync_prod_srv' -e 'ssh' {$paths['gitLocalRepository']} {$paths['rsyncRemoteUser']}:{$paths['rsyncRemotePath']};", $rsynclog);
